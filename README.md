...
| --- | --- | --- |
| **tracehound** | Passive network sensor. Assembles bidirectional flows from a pcap or a live interface, fingerprints TLS clients with JA4, and maps seven detectors to MITRE ATT&CK: C2 beaconing, DNS tunnelling, port scans, host sweeps, exfiltration, new devices, rare TLS stacks. Every alert carries the numbers that produced it. One static binary, no cgo, no libpcap. | Go |
